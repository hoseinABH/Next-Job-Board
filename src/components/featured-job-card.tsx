...
export default function FeaturedJobCard({ className, job }: Props) {
  return (
    <div
      className={cn(
        'flex flex-col relative w-full items-center justify-between shadow-md hover:shadow-none rounded-md px-8 py-10 bg-background dark:bg-secondary/30 transition-all hover:scale-95',
        className
      )}
    >
      {job.isUrgent ? (
